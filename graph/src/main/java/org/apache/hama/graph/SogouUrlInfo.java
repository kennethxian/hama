...
public class SogouUrlInfo{
  ...
  public SogouUrlInfo( String url ){
        b = url.getBytes();
  ...
  static final String[] FIRST_PAGE_NAME = { "index", "default", "main", "home", "homepage" };

  public static boolean checkIndexURL( String url ) {
    int purlstart = 0;
    int purlend   = url.length();
    if ( purlend > purlstart ) {
      String filename = new String();
      int ifpara = 0;
      getUrlFilename( url, filename, 16, ifpara );
      if ( ifpara == 1 ) {          
        return false;
      }
      for ( int i = 0; i < FIRST_PAGE_NAME.length; i ++ ) {
        if ( filename.compareTo( FIRST_PAGE_NAME[i] ) == 0 ) {
          return true;
        }
      }
    }
  ...
  static int getUrlFilename( String url, String filename, int fnsize, Integer ifpara ) {
    ifpara = 0;
    int pscan = 8;      //skip protocol head
    int ppathend = url.length();
    int pfnend = url.length();
    while ( pscan < url.length() ) {
      if ( url.charAt( pscan ) == '/' ) {
        ppathend = pscan;
      }
      else if ( url.charAt( pscan ) == '?' && url.charAt( pscan-1 ) != '/' || (url.charAt( pscan ) == ';') ) {
        ifpara = 1;
        break;
      }
      else if ( url.charAt( pscan ) == '.' ) {
        pfnend = pscan;
      }
      pscan ++;
    }
    int fnamelen = 0;
    int pfilename = ppathend + 1;
    if ( pfilename < pfnend ) {
      fnamelen = pfnend - pfilename;
      if ( fnamelen < fnsize ) {
        while ( pfilename < pfnend ) {
          filename += url.charAt(pfilename);
          pfilename ++;
        }
      }
      else {
